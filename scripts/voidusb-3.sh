...
case $step in
  ...
  3)
    ...
    lsblk
    echo -ne "\nSelect the partition to mount (eg: sda1) : "
    read x;
    mount /dev/$x /media
    lsblk
    echo -ne "\n\n\nADD USER\n"
    ...
    passwd daisn
    echo -ne "\n\n\nedit visudo and change user to daisn\n"
    exit
    ;;
  ...
  4)
    ...
    cat ~/git/scripts/pkg | xargs sudo xbps-install -Sy
    ...
    cd ~/.config/
    git clone https://gitlab.com/daisn/nvim.git
    echo -ne "\n\n\nINSTALLING DWM...\n"
